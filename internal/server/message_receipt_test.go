...
func TestMessageReceiptRoundTrip(t *testing.T) {
	...
	for _, uid := range []string{"receipt-sender", "receipt-reader"} {
	...
	sender, reader := connect("receipt-sender"), connect("receipt-reader")
	defer sender.Close()
	defer reader.Close()
	sendJSON(t, sender, jsonrpc.SendRequest{BaseRequest: jsonrpc.BaseRequest{Method: jsonrpc.MethodSend, ID: "receipt-send"}, Params: jsonrpc.SendParams{
		ChannelID: "receipt-reader", ChannelType: 1, Setting: jsonrpc.SettingFlags{Receipt: true}, Payload: json.RawMessage(`{"type":1,"content":"receipt test"}`)}})
	...
	body := map[string]interface{}{"uid": "receipt-reader", "channel_id": "receipt-sender", "channel_type": 1, "message_seqs": []uint32{msg.Params.MessageSeq}}
	post("/message/receipt", "test-receipt-sender", body, 401)
	post("/message/receipt", "", body, 401)
	post("/message/receipt", "test-receipt-reader", body, 200)
	...
	require.Equal(t, "receipt-reader", command.Param.ChannelID)
	syncBody := map[string]interface{}{"uid": "receipt-sender", "channel_id": "receipt-reader", "channel_type": 1, "extra_version": 0, "limit": 100}
	var page []wkdb.MessageReceipt
	require.NoError(t, json.Unmarshal(post("/message/extra/sync", "test-receipt-sender", syncBody, 200), &page))
	...
	require.JSONEq(t, `[]`, string(post("/message/extra/sync", "test-receipt-sender", syncBody, 200)))
}
